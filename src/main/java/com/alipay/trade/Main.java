...
//        Configs.init("zfbinfo.properties");
    ...
    private void dumpResponse(AlipayResponse response) {
        if (response != null) {
            log.debug(String.format("code:%s, msg:%s", response.getCode(), response.getMsg()));
            if (StringUtils.isNotEmpty(response.getSubCode())) {
                log.debug(String.format("subCode:%s, subMsg:%s", response.getSubCode(),
                    response.getSubMsg()));
            }
...
//            .setGoodsDetailList(goodsDetailList).setTimeoutExpress(timeoutExpress);
    ...
    public void test_trade_query() {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                log.debug("查询返回该订单支付成功: )");

                AlipayTradeQueryResponse response = result.getResponse();
                dumpResponse(response);

                log.debug(response.getTradeStatus());
                if (Utils.isListNotEmpty(response.getFundBillList())) {
                    for (TradeFundBill bill : response.getFundBillList()) {
                        log.debug(bill.getFundChannel() + ":" + bill.getAmount());
                    }
                }
                break;
